...
class PropertiesInspector extends Controller {
  ...
  clear() {
    const html = [],
      localizedText =
        this.viewer.localeService.translate('propertiesInspector.noObjectSelectedWarning') ||
    ...
    html.push(
      `<p class="xeokit-i18n subsubtitle no-object-selected-warning" data-xeokit-i18n="propertiesInspector.noObjectSelectedWarning">${localizedText}</p>`
    )
    html.push(`</div>`)
  ...
  _setPropertySets(metaObject, propertySets) {
    ...
    if (!metaObject) {
      ...
      html.push(
        `<tr><td class="td1">UUID:</td><td class="td2">${metaObject.originalSystemId}</td></tr>`
      )
      ...
      if (attributes) {
        for (let key in attributes) {
          html.push(
            `<tr><td class="td1">${capitalizeFirstChar(key)}:</td><td class="td2">${attributes[key]}</td></tr>`
          )
        }
      }
      html.push('</table>')
      if (!propertySets || propertySets.length === 0) {
        const localizedText =
          this.viewer.localeService.translate('propertiesInspector.noPropSetWarning') ||
          'No properties sets found for this object'
        html.push(
          `<p class="xeokit-i18n subtitle xeokit-no-prop-set-warning" data-xeokit-i18n="propertiesInspector.noPropSetWarning">${localizedText}</p>`
        )
        html.push(`</div>`)
      } else {
        html.push(`</div>`)
        ...
        for (let i = 0, len = propertySets.length; i < len; i++) {
          ...
          if (properties.length > 0) {
            ...
            for (let i = 0, len = properties.length; i < len; i++) {
              ...
              html.push(
                `<tr><td class="td1">${property.name || property.label}:</td><td class="td2">${property.value}</td></tr>`
              )
            }
