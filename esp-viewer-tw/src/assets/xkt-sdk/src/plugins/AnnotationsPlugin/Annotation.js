  ...
  constructor(owner, cfg) {
    ...
    if (cfg.markerElement) {
      ...
      this._marker.addEventListener(
        'click',
        (this._onMouseClickedExternalMarker = () => {
          this.plugin.fire('markerClicked', this)
        })
      )
      this._marker.addEventListener(
        'mouseenter',
        (this._onMouseEnterExternalMarker = () => {
          this.plugin.fire('markerMouseEnter', this)
        })
      )
      this._marker.addEventListener(
        'mouseleave',
        (this._onMouseLeaveExternalMarker = () => {
    ...
    this._onTick = this.scene.on('tick', () => {
      ...
      if (this._visibilityDirty) {
        ...
        this._label.style.visibility =
          this.visible && this._markerShown && this._labelShown ? 'visible' : 'hidden'
