  ...
  for (let i = 0, len = metaObjects.length; i < len; i++) {
    ...
    if (metaObject.parent !== undefined && metaObject.parent !== null) {
      ...
      if (metaObject.type === metaObjectParent.type) {
        ...
        while (
          rootMetaObject.parent &&
          metaObjectsMap[rootMetaObject.parent].type === rootMetaObject.type
        ) {
          rootMetaObject = metaObjectsMap[rootMetaObject.parent]
        }
        const rootStats =
          eachRootStats[rootMetaObject.id] ||
          (eachRootStats[rootMetaObject.id] = {
            numChildren: 0,
            countChildren: 0
          })
...
function loadTextureSet(ctx, material) {
  ...
  if (material.pbrMetallicRoughness) {
    ...
    const baseColorTexture =
      pbrMetallicRoughness.baseColorTexture || pbrMetallicRoughness.colorTexture
    if (baseColorTexture) {
    ...
    if (metallicPBR.metallicRoughnessTexture) {
      textureSetCfg.metallicRoughnessTextureId =
        metallicPBR.metallicRoughnessTexture.texture._textureId
    }
  }
  ...
  if (
    textureSetCfg.normalTextureId !== undefined ||
...
function loadMaterialAttributes(ctx, material) {
...
function loadNode(ctx, node, depth, matrix) {
  ...
  if (
    ((nodeName !== undefined && nodeName !== null) || depth === 0) &&
    deferredMeshIds.length > 0
  ) {
    if (nodeName === undefined || nodeName === null) {
      ctx.log(
        `Warning: 'name' properties not found on glTF scene nodes - will randomly-generate object IDs in XKT`
      )
    }
