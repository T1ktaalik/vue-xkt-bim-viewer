...
class BCFViewpointsPlugin extends Plugin {
  ...
  getViewpoint(options = {}) {
    ...
    const coloringMap = Object.values(scene.objects)
      .filter(
        (entity) =>
          opacityObjectIds.has(entity.id) ||
          colorizedObjectIds.has(entity.id) ||
          xrayedObjectIds.has(entity.id)
      )
      .reduce((coloringMap, entity) => {
        ...
        if (entity.xrayed) {
          ...
          alpha = Math.round(entity.opacity * 255)
            .toString(16)
            .padStart(2, '0')
          color = alpha + color
        }
  ...
  setViewpoint(bcfViewpoint, options = {}) {
    ...
    if (bcfViewpoint.components) {
      if (bcfViewpoint.components.visibility) {
        if (!bcfViewpoint.components.visibility.default_visibility) {
          ...
          if (bcfViewpoint.components.visibility.exceptions) {
            bcfViewpoint.components.visibility.exceptions.forEach((component) =>
              this._withBCFComponent(options, component, (entity) => (entity.visible = true))
            )
          }
        } else {
          ...
          if (bcfViewpoint.components.visibility.exceptions) {
            bcfViewpoint.components.visibility.exceptions.forEach((component) =>
              this._withBCFComponent(options, component, (entity) => (entity.visible = false))
            )
          }
        }
      ...
      if (bcfViewpoint.components.selection) {
        scene.setObjectsSelected(scene.selectedObjectIds, false)
        bcfViewpoint.components.selection.forEach((component) =>
          this._withBCFComponent(options, component, (entity) => (entity.selected = true))
        )
      }

      if (bcfViewpoint.components.translucency) {
        scene.setObjectsXRayed(scene.xrayedObjectIds, false)
        bcfViewpoint.components.translucency.forEach((component) =>
          this._withBCFComponent(options, component, (entity) => (entity.xrayed = true))
        )
      }
...
function colorizeToRGB(color) {
  let rgb = ''
  rgb += Math.round(color[0] * 255)
    .toString(16)
    .padStart(2, '0')
  rgb += Math.round(color[1] * 255)
    .toString(16)
    .padStart(2, '0')
  rgb += Math.round(color[2] * 255)
    .toString(16)
    .padStart(2, '0')
  return rgb
}
