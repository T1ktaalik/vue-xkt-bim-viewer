        ...
        if (instanceRep.referenceReps) {
          for (var id in instanceRep.referenceReps) {
            ...
            for (var id2 in referenceRep) {
              ...
              var material = lines
                ? ctx.modelNode._wireframeMaterial
                : meshCfg.materialId
                  ? ctx.materials[meshCfg.materialId]
                  : null
...
function loadCATMaterialRefDocument(ctx, src, ok) {
  // Loads CATMaterialRef.3dxml
  ctx.zip.getFile(src, function (xmlDoc, json) {
    parseCATMaterialRefDocument(ctx, json, ok)
  })
}

function parseCATMaterialRefDocument(ctx, node, ok) {
  // Parse CATMaterialRef.3dxml
...
function parseModel_3dxml(ctx, node, ok) {
  // Parse CATMaterialRef.3dxml
