...
export class FaceAlignedSectionPlanesPlugin extends Plugin {
  ...
  constructor(viewer, cfg = {}) {
    ...
    if (cfg.overviewCanvasId !== null && cfg.overviewCanvasId !== undefined) {
      ...
      if (!overviewCanvas) {
        this.warn(
          "Can't find overview canvas: '" +
            cfg.overviewCanvasId +
            "' - will create plugin without overview"
        )
      } else {
    ...
    if (cfg.controlElementId === null || cfg.controlElementId === undefined) {
      ...
      if (!this._controlElement) {
        this.warn(
          "Can't find control element: '" +
            cfg.controlElementId +
            "' - will create plugin without control element"
        )
      }
    }
  ...
  _sectionPlaneCreated(sectionPlane) {
    const control =
      this._freeControls.length > 0
        ? this._freeControls.pop()
        : new FaceAlignedSectionPlanesControl(this)
