...
function inflate(deflatedData) {
  return {
    ...
    instancedPositionsDecodeMatrix: new Float32Array(
      pako.inflate(deflatedData.instancedPositionsDecodeMatrix).buffer
    ),
    batchedPositionsDecodeMatrix: new Float32Array(
      pako.inflate(deflatedData.batchedPositionsDecodeMatrix).buffer
    ),
...
function load(viewer, options, inflatedData, sceneModel, metaModel, manifestCtx) {
  ...
  for (let i = 0; i < numEntities; i++) {
    ...
    for (
      let j = entityMeshes[i], jlen = lastEntity ? entityMeshIds.length : entityMeshes[i + 1];
      j < jlen;
      j++
    ) {
      ...
      var tmpPositions = positions.subarray(
        meshPositions[jj],
        lastMesh ? positions.length : meshPositions[jj + 1]
      )
      var tmpNormals = normals.subarray(
        meshPositions[jj],
        lastMesh ? positions.length : meshPositions[jj + 1]
      )
      var tmpIndices = indices.subarray(
        meshIndices[jj],
        lastMesh ? indices.length : meshIndices[jj + 1]
      )
      var tmpEdgeIndices = edgeIndices.subarray(
        meshEdgesIndices[jj],
        lastMesh ? edgeIndices.length : meshEdgesIndices[jj + 1]
      )
