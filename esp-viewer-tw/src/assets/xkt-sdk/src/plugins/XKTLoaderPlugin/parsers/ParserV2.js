...
function inflate(deflatedData) {
  return {
    ...
    positionsDecodeMatrix: new Float32Array(
      pako.inflate(deflatedData.positionsDecodeMatrix).buffer
    ),
...
function load(viewer, options, inflatedData, sceneModel, metaModel, manifestCtx) {
  ...
  for (let i = 0; i < numEntities; i++) {
    ...
    for (
      ...
      const tmpPositions = positions.subarray(
        meshPositions[jj],
        lastMesh ? positions.length : meshPositions[jj + 1]
      )
      const tmpNormals = normals.subarray(
        meshPositions[jj],
        lastMesh ? positions.length : meshPositions[jj + 1]
      )
      const tmpIndices = indices.subarray(
        meshIndices[jj],
        lastMesh ? indices.length : meshIndices[jj + 1]
      )
      const tmpEdgeIndices = edgeIndices.subarray(
        meshEdgesIndices[jj],
