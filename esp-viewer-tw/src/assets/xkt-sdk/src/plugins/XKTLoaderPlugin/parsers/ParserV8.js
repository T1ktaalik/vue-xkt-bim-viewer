...
function load(viewer, options, inflatedData, sceneModel, metaModel, manifestCtx) {
  ...
  if (metaModel) {
    ...
    for (let metaObjectIndex = 0; metaObjectIndex < numMetaObjects; metaObjectIndex++) {
      ...
      const metaObjectParentId =
        metaObjectParentIndex !== metaObjectIndex ? eachMetaObjectId[metaObjectParentIndex] : null
  ...
  for (let tileIndex = 0; tileIndex < numTiles; tileIndex++) {
    ...
    for (
      ...
      const lastMeshIndex = atLastTileEntity
        ? eachMeshGeometriesPortion.length
        : eachEntityMeshesPortion[tileEntityIndex + 1]
        ...
        if (isReusedGeometry) {
          ...
          if (!geometryArrays) {
            ...
            switch (primitiveType) {
              ...
              case 2:
                ...
                geometryArrays.geometryColors = convertColorsRGBToRGBA(
                  colors.subarray(
          ...
          switch (primitiveType) {
            ...
            case 2:
              ...
              geometryColors = convertColorsRGBToRGBA(
                colors.subarray(
                  eachGeometryColorsPortion[geometryIndex],
                  atLastGeometry ? colors.length : eachGeometryColorsPortion[geometryIndex + 1]
                )
