...
class DistanceMeasurement extends Component {
  ...
  _update() {
    ...
    if (this._cpDirty) {
      ...
      if (!this.labelsVisible) {
        ...
        this._length = Math.abs(
          math.lenVec3(math.subVec3(this._targetWorld, this._originWorld, distVec3))
        )
        ...
        if (!this._xAxisLabelCulled) {
          this._xAxisLabel.setText(
            tilde +
              Math.abs((this._targetWorld[0] - this._originWorld[0]) * scale).toFixed(2) +
              unitAbbrev
          )
        ...
        if (!this._yAxisLabelCulled) {
          this._yAxisLabel.setText(
            tilde +
              Math.abs((this._targetWorld[1] - this._originWorld[1]) * scale).toFixed(2) +
              unitAbbrev
          )
        ...
        if (!this._zAxisLabelCulled) {
          this._zAxisLabel.setText(
            tilde +
              Math.abs((this._targetWorld[2] - this._originWorld[2]) * scale).toFixed(2) +
              unitAbbrev
          )
  ...
  set labelsVisible(value) {
    ...
    this._xAxisLabel.setVisible(
      labelsVisible && !this._xAxisLabelCulled && this._clickable && this._axisEnabled
    )
    this._yAxisLabel.setVisible(
      labelsVisible && !this._yAxisLabelCulled && this._clickable && this._axisEnabled
    )
    this._zAxisLabel.setVisible(
      labelsVisible && !this._zAxisLabelCulled && this._clickable && this._axisEnabled
    )
