...
class Wire {
  constructor(parentElement, cfg = {}) {
    ...
    this._wireClickable.className += this._wireClickable.className
      ? ' viewer-ruler-wire-clickable'
      : 'viewer-ruler-wire-clickable'
  ...
  _update() {
    var length = Math.abs(
      Math.sqrt(
        (this._x1 - this._x2) * (this._x1 - this._x2) +
          (this._y1 - this._y2) * (this._y1 - this._y2)
      )
    )
