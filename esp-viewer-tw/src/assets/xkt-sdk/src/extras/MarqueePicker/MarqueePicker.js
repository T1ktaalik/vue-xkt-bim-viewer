...
export class MarqueePicker extends Component {
  ...
  setPickMode(pickMode) {
    if (
      ...
      pickMode !== MarqueePicker.PICK_MODE_INTERSECTS
    ) {
  ...
  pick() {
    ...
    if (
      this._canvasMarquee[2] - this._canvasMarquee[0] > 3 ||
      this._canvasMarquee[3] - this._canvasMarquee[1] > 3
    ) {
      // Marquee pick if rectangle big enough
  ...
  _buildMarqueeFrustum() {
    // https://github.com/xeokit/xeokit-sdk/issues/869#issuecomment-1165375770
    ...
    setFrustum(
      this._marqueeFrustum,
      this.viewer.scene.camera.viewMatrix,
      this._marqueeFrustumProjMat
    )
  }
