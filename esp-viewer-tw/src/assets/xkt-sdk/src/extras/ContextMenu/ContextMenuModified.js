...
class ContextMenu {
  ...
  _clear() {
    // Destroys DOM elements, clears menu data
  ...
  _createMenuUI(menu) {
    // Builds DOM elements for a menu
    const groups = menu.groups
    const html = []

    const $ = cheerio.load(
      `<div class="xeokit-context-menu ${menu.id}" style="z-index:300000; position: absolute;"></div>`
    )
      ...
      for (let i = 0, len = groups.length; i < len; i++) {
        ...
        if (groupItems) {
          for (let j = 0, lenj = groupItems.length; j < lenj; j++) {
            ...
            if (itemSubMenu) {
              $('ul').append(
                `<li id="${item.id}" class="xeokit-context-menu-item  " style="${groupIdx === groupLen - 1 || j < lenj - 1 ? 'border-bottom: 0' : 'border-bottom: 1px solid black'}">${actionTitle} [MORE]</li> `
              )
            } else {
              ...
              $('ul').append(
                `<li id="${item.id}" class="xeokit-context-menu-item  ${groupIdx === groupLen - 1 || j < lenj - 1 ? '' : 'tw-border-solid tw-border tw-border-b-gray-600'}" style="">${actionTitle} </li>`
              )
            }
          }
        }
    ...
    $('div').addClass('tw-hidden tw-shadow-orange-100 tw-bg-orange-50/80 tw-text-gray-600 tw-p-1 ')
    $('li').addClass(
      'hover:tw-text-blue-700 hover:tw-cursor-pointer tw-font-light hover:tw-font-normal'
    )
  ...
  _updateItemsTitles() {
    // Dynamically updates the title of each Item to the result of Item#getTitle()
    if (!this._context) {
      return
    }
  ...
  _updateItemsEnabledStatus() {
    // Enables or disables each Item, depending on the result of Item#getEnabled()
    if (!this._context) {
      return
    }
    for (let i = 0, len = this._itemList.length; i < len; i++) {
  ...
  _showMenu(menuId, pageX, pageY) {
    // Shows the given menu, at the specified page coordinates
  ...
  _showMenuElement(menuElement, pageX, pageY) {
    // Shows the given menu element, at the specified page coordinates
