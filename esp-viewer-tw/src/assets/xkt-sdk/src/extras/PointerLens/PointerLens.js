...
export class PointerLens {
  ...
  constructor(viewer, cfg = {}) {
    ...
    this.viewer.scene.canvas.canvas.parentNode.insertBefore(
      this._lensCursorDiv,
      this.viewer.scene.canvas.canvas
    )
  ...
  update() {
    ...
    const pointerOnLens =
      this._canvasPos[0] < lensRect.right &&
      this._canvasPos[0] > lensRect.left &&
      this._canvasPos[1] < lensRect.bottom &&
      this._canvasPos[1] > lensRect.top
    this._lensContainer.style.marginLeft = `25px`
    if (pointerOnLens) {
