...
class MetaScene {
  ...
  createMetaModel(modelId, metaModelData, options = {}) {
    const metaModel = new MetaModel({
      // Registers MetaModel in #metaModels
      metaScene: this,
      id: modelId,
  ...
  destroyMetaModel(id) {
    ...
    if (metaModel.propertySets) {
      for (let i = 0, len = metaModel.propertySets.length; i < len; i++) {
        ...
        if (propertySet.metaModels.length === 1) {
          // Property set owned by one model, delete
    ...
    if (metaModel.metaObjects) {
      for (let i = 0, len = metaModel.metaObjects.length; i < len; i++) {
        ...
        if (metaObject.metaModels.length === 1) {
          // MetaObject owned by one model, delete
