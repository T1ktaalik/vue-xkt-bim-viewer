...
class Input extends Component {
  ...
  _bindEvents() {
    ...
    this._keyboardEventsElement.addEventListener(
      'keydown',
    ...
    const tickifiedMouseWheelFn = this.scene.tickify((delta) => {
      this.fire('mousewheel', delta, true)
    })

    this.element.addEventListener(
      'wheel',
      (this._mouseWheelListener = (e, d) => {
        if (!this.enabled) {
          return
        }
