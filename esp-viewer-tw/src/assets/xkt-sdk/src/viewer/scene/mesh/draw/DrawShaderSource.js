...
function hasNormals(mesh) {
  ...
  if (
    (mesh._geometry._state.autoVertexNormals || mesh._geometry._state.normalsBuf) &&
...
function buildVertexLambert(mesh) {
  ...
  if (geometryState.normalsBuf) {
    src.push(
      'vec3 viewNormal = normalize((viewNormalMatrix2 * modelNormalMatrix2 * localNormal).xyz);'
    )
  }
  ...
  src.push(
    'vColor = vec4((lightAmbient.rgb * lightAmbient.a * materialColor.rgb) + materialEmissive.rgb + (reflectedColor * materialColor.rgb), materialColor.a) * colorize;'
  ) // TODO: How to have ambient bright enough for canvas BG but not too bright for scene?
  if (clipping) {
...
function buildFragmentLambert(mesh) {
  ...
  if (scene.logarithmicDepthBufferEnabled) {
    src.push(
      'gl_FragDepth = isPerspective == 0.0 ? gl_FragCoord.z : log2( vFragDepth ) * logDepthBufFC * 0.5;'
    )
  }
  if (gammaOutput) {
    ...
    for (let i = 0, len = lightsState.lights.length; i < len; i++) {
      ...
      if (lightsState.lights[i].castsShadow) {
        src.push(
          'vShadowPosFromLight' +
            i +
            ' = texUnitConverter * shadowProjMatrix' +
            i +
            ' * (shadowViewMatrix' +
            i +
            ' * worldPosition); '
...
function buildFragmentDraw(mesh) {
  ...
  if (normals) {
    ...
    if (phongMaterial) {
      if (lightsState.lightMaps.length > 0 || lightsState.reflectionMaps.length > 0) {
        src.push(
          'void computePhongLightMapping(const in Geometry geometry, const in Material material, inout ReflectedLight reflectedLight) {'
        )

        if (lightsState.lightMaps.length > 0) {
          src.push(
            '   vec3 irradiance = ' +
              TEXTURE_DECODE_FUNCS[lightsState.lightMaps[0].encoding] +
              '(texture(lightMap, geometry.worldNormal)).rgb;'
          )
      ...
      src.push(
        'void computePhongLighting(const in IncidentLight directLight, const in Geometry geometry, const in Material material, inout ReflectedLight reflectedLight) {'
      )
      ...
      src.push(
        '   reflectedLight.diffuse  += irradiance * (RECIPROCAL_PI * material.diffuseColor);'
      )
      src.push(
        '   reflectedLight.specular += directLight.color * material.specularColor * pow(max(dot(reflect(-directLight.direction, -geometry.viewNormal), geometry.viewEyeDir), 0.0), material.shine);'
      )
      src.push('}')
    }
    ...
    if (metallicMaterial || specularMaterial) {
      ...
      src.push(
        '   float desiredMIPLevel = maxMIPLevelScalar - 0.79248 - 0.5 * log2( ( blinnShininessExponent * blinnShininessExponent ) + 1.0 );'
      )
      ...
      if (lightsState.reflectionMaps.length > 0) {
        ...
        src.push(
          '   vec3 envMapColor = ' +
            TEXTURE_DECODE_FUNCS[lightsState.reflectionMaps[0].encoding] +
            '(texture(reflectionMap, reflectVec, mipLevel)).rgb;'
        )
      ...
      if (lightsState.lightMaps.length > 0 || lightsState.reflectionMaps.length > 0) {
        src.push(
          'void computePBRLightMapping(const in Geometry geometry, const in Material material, inout ReflectedLight reflectedLight) {'
        )
        if (lightsState.lightMaps.length > 0) {
          src.push(
            '   vec3 irradiance = sRGBToLinear(texture(lightMap, geometry.worldNormal)).rgb;'
          )
          src.push('   irradiance *= PI;')
          src.push('   vec3 diffuseBRDFContrib = (RECIPROCAL_PI * material.diffuseColor);')
          src.push('   reflectedLight.diffuse += irradiance * diffuseBRDFContrib;')
  ...
  if (
    ...
    src.push(
      'float fresnel(vec3 eyeDir, vec3 normal, float edgeBias, float centerBias, float power) {'
    )
  ...
  if (uvs && material._ambientMap) {
    ...
    src.push(
      'ambientTexel = ' +
        TEXTURE_DECODE_FUNCS[material._ambientMap._state.encoding] +
        '(ambientTexel);'
    )
  ...
  if (uvs && material._diffuseMap) {
    ...
    src.push(
      'diffuseTexel = ' +
        TEXTURE_DECODE_FUNCS[material._diffuseMap._state.encoding] +
        '(diffuseTexel);'
    )
  ...
  if (uvs && material._baseColorMap) {
    ...
    src.push(
      'baseColorTexel = ' +
        TEXTURE_DECODE_FUNCS[material._baseColorMap._state.encoding] +
        '(baseColorTexel);'
    )
  ...
  if (uvs && material._emissiveMap) {
    ...
    src.push(
      'emissiveTexel = ' +
        TEXTURE_DECODE_FUNCS[material._emissiveMap._state.encoding] +
        '(emissiveTexel);'
    )
  ...
  if (
    normals &&
    (lightsState.lights.length > 0 ||
      lightsState.lightMaps.length > 0 ||
      lightsState.reflectionMaps.length > 0)
  ) {
    //--------------------------------------------------------------------------------
    // SHADING
    //--------------------------------------------------------------------------------
    ...
    if (uvs && material._normalMap) {
      ...
      src.push(
        'vec3 viewNormal = perturbNormal2Arb( vViewPosition, normalize(vViewNormal), textureCoord );'
      )
    } else {
    ...
    if (material._diffuseFresnel) {
      src.push(
        'float diffuseFresnel = fresnel(viewEyeDir, viewNormal, diffuseFresnelEdgeBias, diffuseFresnelCenterBias, diffuseFresnelPower);'
      )
      src.push(
        'diffuseColor *= mix(diffuseFresnelEdgeColor, diffuseFresnelCenterColor, diffuseFresnel);'
      )
    }
    if (material._specularFresnel) {
      src.push(
        'float specularFresnel = fresnel(viewEyeDir, viewNormal, specularFresnelEdgeBias, specularFresnelCenterBias, specularFresnelPower);'
      )
      src.push(
        'specular *= mix(specularFresnelEdgeColor, specularFresnelCenterColor, specularFresnel);'
      )
    }
    if (material._alphaFresnel) {
      src.push(
        'float alphaFresnel = fresnel(viewEyeDir, viewNormal, alphaFresnelEdgeBias, alphaFresnelCenterBias, alphaFresnelPower);'
      )
      src.push('alpha *= mix(alphaFresnelEdgeColor.r, alphaFresnelCenterColor.r, alphaFresnel);')
    }
    if (material._emissiveFresnel) {
      src.push(
        'float emissiveFresnel = fresnel(viewEyeDir, viewNormal, emissiveFresnelEdgeBias, emissiveFresnelCenterBias, emissiveFresnelPower);'
      )
      src.push(
        'emissiveColor *= mix(emissiveFresnelEdgeColor, emissiveFresnelCenterColor, emissiveFresnel);'
      )
    }
    ...
    if (
      phongMaterial &&
      (lightsState.lightMaps.length > 0 || lightsState.reflectionMaps.length > 0)
    ) {
      src.push('computePhongLightMapping(geometry, material, reflectedLight);')
    }

    if (
      (specularMaterial || metallicMaterial) &&
      (lightsState.lightMaps.length > 0 || lightsState.reflectionMaps.length > 0)
    ) {
    ...
    for (let i = 0, len = lightsState.lights.length; i < len; i++) {
      ...
      if (receivesShadow && light.castsShadow) {
        ...
        src.push(
          '      float texelDepth = unpackDepth(texture(shadowMap' +
            i +
    ...
    if (phongMaterial) {
      src.push(
        'vec3 outgoingLight = (lightAmbient.rgb * lightAmbient.a * diffuseColor) + ((occlusion * (( reflectedLight.diffuse + reflectedLight.specular)))) + emissiveColor;'
      )
    } else {
      src.push(
