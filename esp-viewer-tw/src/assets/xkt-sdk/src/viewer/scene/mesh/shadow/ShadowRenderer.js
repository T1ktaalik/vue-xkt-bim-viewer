...
ShadowRenderer.prototype.drawMesh = function (frame, mesh) {
  ...
  if (geometryState.combineGeometry) {
    ...
    if (vertexBufs.id !== this._lastVertexBufsId) {
      if (vertexBufs.positionsBuf && this._aPosition) {
        this._aPosition.bindArrayBuffer(
          vertexBufs.positionsBuf,
          vertexBufs.compressGeometry ? gl.UNSIGNED_SHORT : gl.FLOAT
        )
        frame.bindArray++
      }
  ...
  if (geometryState.id !== this._lastGeometryId) {
    ...
    if (geometryState.combineGeometry) {
      // VBOs were bound by the preceding VertexBufs chunk
      ...
      if (this._aPosition) {
        this._aPosition.bindArrayBuffer(
          geometryState.positionsBuf,
          geometryState.compressGeometry ? gl.UNSIGNED_SHORT : gl.FLOAT
        )
        frame.bindArray++
      }
  ...
  if (geometryState.combineGeometry) {
    if (geometryState.indicesBufCombined) {
      gl.drawElements(
        ...
        geometryState.indicesBufCombined.numItems,
        geometryState.indicesBufCombined.itemType,
        0
      )
      frame.drawElements++
    } else {
