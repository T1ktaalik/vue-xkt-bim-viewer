...
export class DTXTrianglesNormalsRenderer {
  ...
  drawLayer(frameCtx, dataTextureLayer, renderPass) {
    ...
    gl.uniformMatrix4fv(
      ...
      false,
      dataTextureLayer._state.objectDecodeAndInstanceMatrix
    )
  ...
  _allocate() {
    ...
    if (this._aFlags2) {
      // Won't be in shader when not clipping
  ...
  _buildVertexShader() {
    ...
    src.push('  } else {')
    src.push(
      '      vec4 worldPosition = worldMatrix * (objectDecodeAndInstanceMatrix * vec4(position, 1.0)); '
    )
    if (scene.entityOffsetsEnabled) {
  ...
  _buildFragmentShader() {
    ...
    if (scene.logarithmicDepthBufferEnabled && WEBGL_INFO.SUPPORTED_EXTENSIONS['EXT_frag_depth']) {
      src.push(
        '    gl_FragDepthEXT = isPerspective == 0.0 ? gl_FragCoord.z : log2( vFragDepth ) * logDepthBufFC * 0.5;'
      )
    }
