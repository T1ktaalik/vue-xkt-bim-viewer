...
export class DTXTrianglesLayer {
  ...
  canCreatePortion(portionCfg) {
    ...
    const bucketGeometryId =
      portionCfg.geometryId !== undefined && portionCfg.geometryId !== null
    ...
    if (!alreadyHasPortionGeometry) {
      const maxIndicesOfAnyBits = Math.max(
        this._state.numIndices8Bits,
        this._state.numIndices16Bits,
        this._state.numIndices32Bits
      )
      let numVertices = 0
      let numIndices = 0
  ...
  _createBucketGeometry(portionCfg, bucket) {
    ...
    if (bucket.indices) {
      const alignedIndicesLen =
        Math.ceil(bucket.indices.length / 3 / INDICES_EDGE_INDICES_ALIGNEMENT_SIZE) *
        INDICES_EDGE_INDICES_ALIGNEMENT_SIZE *
        3
      dataTextureRamStats.overheadSizeAlignementIndices +=
        2 * (alignedIndicesLen - bucket.indices.length)
    ...
    if (bucket.edgeIndices) {
      const alignedEdgeIndicesLen =
        Math.ceil(bucket.edgeIndices.length / 2 / INDICES_EDGE_INDICES_ALIGNEMENT_SIZE) *
        INDICES_EDGE_INDICES_ALIGNEMENT_SIZE *
        2
      dataTextureRamStats.overheadSizeAlignementEdgeIndices +=
        2 * (alignedEdgeIndicesLen - bucket.edgeIndices.length)
  ...
  _createSubPortion(portionCfg, bucketGeometry, bucket, subPortionAABB) {
    ...
    if (colors) {
      ...
      // Color is pre-quantized by SceneModel
...

    {
      ...
      buffer.perObjectEdgeIndexBaseOffsets.push(
        currentNumEdgeIndices / 2 - bucketGeometry.edgeIndicesBase
      )
    }
  ...
  finalize() {
    ...
    textureState.texturePerObjectColorsAndFlags =
      this._dtxTextureFactory.createTextureForColorsAndFlags(
        gl,
    ...
    textureState.texturePerObjectInstanceMatrices =
      this._dtxTextureFactory.createTextureForInstancingMatrices(
        gl,
  ...
  _subPortionSetFlags(subPortionId, flags, transparent, deferred = false) {
    ...
    // object flags
    textureState.texturePerObjectColorsAndFlags._textureData.set(
      tempUint8Array4,
      subPortionId * 32 + 8
    )
  ...
  drawEdgesColorOpaque(renderFlags, frameCtx) {
    if (this.model.scene.logarithmicDepthBufferEnabled) {
      if (!this.model.scene._loggedWarning) {
        console.log(
          'Edge enhancement for SceneModel data texture layers currently disabled with logarithmic depth buffer'
        )
