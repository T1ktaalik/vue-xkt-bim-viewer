...
export class VBOBatchingTrianglesLayer {
  ...
  canCreatePortion(lenPositions, lenIndices) {
    ...
    return (
      this._buffer.positions.length + lenPositions < this._buffer.maxVerts * 3 &&
      this._buffer.indices.length + lenIndices < this._buffer.maxIndices
    )
  }
  ...
  finalize() {
    ...
    if (buffer.positions.length > 0) {
      const quantizedPositions = this._state.positionsDecodeMatrix
        ...
        : quantizePositions(
            buffer.positions,
            this._modelAABB,
            (this._state.positionsDecodeMatrix = math.mat4())
          ) // BOTTLENECK
      state.positionsBuf = new ArrayBuf(
        gl,
        gl.ARRAY_BUFFER,
        quantizedPositions,
        quantizedPositions.length,
        3,
        gl.STATIC_DRAW
      )
    ...
    if (buffer.metallicRoughness.length > 0) {
      ...
      state.metallicRoughnessBuf = new ArrayBuf(
        ...
        gl.ARRAY_BUFFER,
        metallicRoughness,
        buffer.metallicRoughness.length,
        2,
        gl.STATIC_DRAW,
        normalized
      )
    ...
    if (buffer.positions.length > 0) {
      // Because we build flags arrays here, get their length from the positions array
    ...
    this._state.colorTextureSupported =
      !!state.uvBuf && !!state.textureSet && !!state.textureSet.colorTexture
