...
function rebucketPositions(mesh, bitsPerBucket, checkResult = false) {
  ...
  const originalSize =
    positionsCompressed.length * 2 + (indices.length + edgeIndices.length) * prevBytesPerIndex
  ...
  buckets.forEach((bucket) => {
    newSize +=
      bucket.positionsCompressed.length * 2 +
      (bucket.indices.length + bucket.edgeIndices.length) * newBytesPerIndex
...
function doCheckResult(buckets, mesh) {
  ...
  buckets.forEach((bucket) => {
    ...
    for (let i = 0, len = indices.length; i < len; i += 3) {
      const key =
        positionsCompressed[indices[i] * 3] +
        '_' +
        positionsCompressed[indices[i] * 3 + 1] +
        '_' +
        positionsCompressed[indices[i] * 3 + 2] +
        '/' +
        positionsCompressed[indices[i + 1] * 3] +
        '_' +
        positionsCompressed[indices[i + 1] * 3 + 1] +
        '_' +
        positionsCompressed[indices[i + 1] * 3 + 2] +
        '/' +
        positionsCompressed[indices[i + 2] * 3] +
        '_' +
        positionsCompressed[indices[i + 2] * 3 + 1] +
        '_' +
        positionsCompressed[indices[i + 2] * 3 + 2]
      meshDict[key] = true
    }
    ...
    for (let i = 0, len = edgeIndices.length; i < len; i += 2) {
      const key =
        positionsCompressed[edgeIndices[i] * 3] +
        '_' +
        positionsCompressed[edgeIndices[i] * 3 + 1] +
        '_' +
        positionsCompressed[edgeIndices[i] * 3 + 2] +
        '/' +
        positionsCompressed[edgeIndices[i + 1] * 3] +
        '_' +
        positionsCompressed[edgeIndices[i + 1] * 3 + 1] +
        '_' +
        positionsCompressed[edgeIndices[i + 1] * 3 + 2] +
        '/'
      edgesDict[key] = true
    }
  })
