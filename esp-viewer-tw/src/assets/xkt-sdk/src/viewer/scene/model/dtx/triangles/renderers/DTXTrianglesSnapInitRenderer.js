...
export class DTXTrianglesSnapInitRenderer {
  ...
  _buildVertexShader() {
    ...
    // get color
    src.push(
      'uvec4 color = texelFetch (uObjectPerObjectColorsAndFlags, ivec2(objectIndexCoords.x*8+0, objectIndexCoords.y), 0);'
    )
    ...
    src.push(
      'vPickColor = vec4(texelFetch (uObjectPerObjectColorsAndFlags, ivec2(objectIndexCoords.x*8+1, objectIndexCoords.y), 0));'
    )
  ...
  _buildFragmentShader() {
    ...
    if (clipping) {
      ...
      for (
        let i = 0, len = scene._sectionPlanesState.getNumAllocatedSectionPlanes();
        i < len;
        i++
      ) {
    ...
    if (SNAPPING_LOG_DEPTH_BUF_ENABLED) {
      ...
      src.push(
        '    gl_FragDepth = isPerspective == 0.0 ? gl_FragCoord.z : log2( vFragDepth + diff ) * logDepthBufFC * 0.5;'
      )
    }
    src.push(
      'outCoords = ivec4(relativeToOriginPosition.xyz * uCoordinateScaler.xyz, - uLayerNumber);'
    )
