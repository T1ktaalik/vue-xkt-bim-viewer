...
export class VBORenderer {
  ...
  _addRemapClipPosLines(src, viewportSize = 1) {
    ...
    if (viewportSize === 1) {
      ...
      src.push(
        `    clipPos.xy = (clipPos.xy - pickClipPos) * (drawingBufferSize / float(${viewportSize}));`
      )
    }
    src.push('    clipPos.xy *= clipPos.w;')
  ...
  drawLayer(
    ...
    layer,
    renderPass,
    { colorUniform = false, incrementDrawState = false } = {}
  ) {
    ...
    if (gotOrigin || gotPosition) {
      ...
      this._matricesUniformBlockBufferData.set(
        createRTCViewMat(viewMatrix, rtcOrigin, tempMat4a),
        (offset += mat4Size)
      )
    } else {
    ...
    this._matricesUniformBlockBufferData.set(
      frameCtx.pickProjMatrix || project.matrix,
      (offset += mat4Size)
    )
      ...
      const nearPlaneHeight =
        scene.camera.projection === 'ortho'
          ...
          : gl.drawingBufferHeight /
            (2 * Math.tan((0.5 * scene.camera.perspective.fov * Math.PI) / 180.0))
    ...
    if (lightsState.lightMaps.length > 0 && lightsState.lightMaps[0].texture && this._uLightMap) {
      this._program.bindTexture(
        this._uLightMap,
        lightsState.lightMaps[0].texture,
        frameCtx.textureUnit
      )
      frameCtx.textureUnit = (frameCtx.textureUnit + 1) % maxTextureUnits
      frameCtx.bindTexture++
    ...
    if (colorUniform) {
      ...
      if (renderPass === RENDER_PASSES[`${this._edges ? 'EDGES' : 'SILHOUETTE'}_XRAYED`]) {
        ...
        renderPass === RENDER_PASSES[`${this._edges ? 'EDGES' : 'SILHOUETTE'}_HIGHLIGHTED`]
      ) {
