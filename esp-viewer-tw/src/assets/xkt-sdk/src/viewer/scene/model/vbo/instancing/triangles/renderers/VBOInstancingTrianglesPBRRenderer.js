...
export class VBOInstancingTrianglesPBRRenderer extends VBOSceneModelTriangleInstancingRenderer {
  ...
  _buildVertexShader() {
    ...
    src.push(
      'vec4 worldNormal = worldNormalMatrix * vec4(dot(modelNormal, modelNormalMatrixCol0), dot(modelNormal, modelNormalMatrixCol1), dot(modelNormal, modelNormalMatrixCol2), 1.0);'
    )
  ...
  _buildFragmentShader() {
    ...
    src.push(
      'vec3 rgb = (vec3(float(vColor.r) / 255.0, float(vColor.g) / 255.0, float(vColor.b) / 255.0));'
    )
    ...
    src.push(
      'vec3 viewNormal = perturbNormal2Arb( vViewPosition.xyz, normalize(vViewNormal), vUV );'
    )

    src.push(
      'material.diffuseColor      = baseColor * (1.0 - dielectricSpecular) * (1.0 - metallic);'
    )
    ...
    src.push(
      'vec3 outgoingLight = (lightAmbient.rgb * lightAmbient.a * baseColor * opacity * rgb) + (reflectedLight.diffuse) + (reflectedLight.specular) + emissiveColor;'
    )
