...
export class VBOInstancingTrianglesLayer {
  ...
  constructor(cfg) {
    ...
    this.sortId =
      ...
      (cfg.normals ? '-normals' : '-autoNormals')
  ...
  finalize() {
    ...
    if (colorsLength > 0) {
      ...
      state.colorsBuf = new ArrayBuf(
        gl,
        gl.ARRAY_BUFFER,
        new Uint8Array(this._colors),
        this._colors.length,
        4,
        gl.DYNAMIC_DRAW,
        notNormalized
    ...
    if (this._metallicRoughness.length > 0) {
      ...
      state.metallicRoughnessBuf = new ArrayBuf(
        gl,
        gl.ARRAY_BUFFER,
        metallicRoughness,
        this._metallicRoughness.length,
        2,
        gl.STATIC_DRAW,
        normalized
      )
    ...
    if (flagsLength > 0) {
      ...
      let notNormalized = false
      state.flagsBuf = new ArrayBuf(
        gl,
        gl.ARRAY_BUFFER,
        new Float32Array(flagsLength),
        flagsLength,
        1,
        gl.DYNAMIC_DRAW,
        notNormalized
    ...
    if (this.model.scene.entityOffsetsEnabled) {
      if (this._offsets.length > 0) {
        ...
        state.offsetsBuf = new ArrayBuf(
          gl,
          gl.ARRAY_BUFFER,
          new Float32Array(this._offsets),
    ...
    if (geometry.positionsCompressed && geometry.positionsCompressed.length > 0) {
      ...
      state.positionsBuf = new ArrayBuf(
        gl,
        gl.ARRAY_BUFFER,
    ...
    if (
      ...
      state.edgeIndicesBuf = new ArrayBuf(
        gl,
        gl.ELEMENT_ARRAY_BUFFER,
        new Uint32Array(geometry.edgeIndices),
    ...
    if (this._modelMatrixCol0.length > 0) {
      ...
      state.modelMatrixCol0Buf = new ArrayBuf(
        gl,
        gl.ARRAY_BUFFER,
        new Float32Array(this._modelMatrixCol0),
        this._modelMatrixCol0.length,
        4,
        gl.STATIC_DRAW,
        normalized
      )
      state.modelMatrixCol1Buf = new ArrayBuf(
        gl,
        gl.ARRAY_BUFFER,
        new Float32Array(this._modelMatrixCol1),
        this._modelMatrixCol1.length,
        4,
        gl.STATIC_DRAW,
        normalized
      )
      state.modelMatrixCol2Buf = new ArrayBuf(
        gl,
        gl.ARRAY_BUFFER,
        new Float32Array(this._modelMatrixCol2),
        this._modelMatrixCol2.length,
        4,
        gl.STATIC_DRAW,
        normalized
      )
      ...
      if (state.normalsBuf) {
        state.modelNormalMatrixCol0Buf = new ArrayBuf(
          gl,
          gl.ARRAY_BUFFER,
          new Float32Array(this._modelNormalMatrixCol0),
          this._modelNormalMatrixCol0.length,
          4,
          gl.STATIC_DRAW,
          normalized
        )
        state.modelNormalMatrixCol1Buf = new ArrayBuf(
          gl,
          gl.ARRAY_BUFFER,
          new Float32Array(this._modelNormalMatrixCol1),
          this._modelNormalMatrixCol1.length,
          4,
          gl.STATIC_DRAW,
          normalized
        )
        state.modelNormalMatrixCol2Buf = new ArrayBuf(
          gl,
          gl.ARRAY_BUFFER,
          new Float32Array(this._modelNormalMatrixCol2),
          this._modelNormalMatrixCol2.length,
          4,
          gl.STATIC_DRAW,
          normalized
        )
    ...
    if (this._pickColors.length > 0) {
      const normalized = false
      state.pickColorsBuf = new ArrayBuf(
        gl,
        gl.ARRAY_BUFFER,
        new Uint8Array(this._pickColors),
        this._pickColors.length,
        4,
        gl.STATIC_DRAW,
        normalized
      )
    ...
    state.colorTextureSupported = !!state.uvBuf && !!textureSet && !!textureSet.colorTexture
  ...
  setColor(portionId, color) {
    // RGBA color is normalized as ints
    if (!this._finalized) {
      throw 'Not finalized'
    }
  ...
  _setFlags(portionId, flags, meshTransparent) {
    ...
    const pickFlag =
      visible && !culled && pickable ? RENDER_PASSES.PICK : RENDER_PASSES.NOT_RENDERED
  ...
  drawColorOpaque(renderFlags, frameCtx) {
    if (
      this._numCulledLayerPortions === this._numPortions ||
      this._numVisibleLayerPortions === 0 ||
      this._numTransparentLayerPortions === this._numPortions ||
    ...
    if (frameCtx.withSAO && this.model.saoEnabled) {
      if (frameCtx.pbrEnabled && this.model.pbrEnabled && this._state.pbrSupported) {
        ...
        frameCtx.colorTextureEnabled &&
        this.model.colorTextureEnabled &&
        this._state.colorTextureSupported
      ) {
        if (this._renderers.colorTextureRendererWithSAO) {
          this._renderers.colorTextureRendererWithSAO.drawLayer(
            frameCtx,
            this,
        ...
        if (this._renderers.flatColorRendererWithSAO) {
          this._renderers.flatColorRendererWithSAO.drawLayer(
            frameCtx,
            this,
            RENDER_PASSES.COLOR_OPAQUE
          )
  ...
  drawColorTransparent(renderFlags, frameCtx) {
    ...
    if (frameCtx.pbrEnabled && this.model.pbrEnabled && this._state.pbrSupported) {
      ...
      if (this._renderers.colorTextureRenderer) {
        this._renderers.colorTextureRenderer.drawLayer(
          frameCtx,
          this,
  ...
  drawSilhouetteHighlighted(renderFlags, frameCtx) {
    ...
    this._updateBackfaceCull(renderFlags, frameCtx)
    if (this._renderers.silhouetteRenderer) {
      this._renderers.silhouetteRenderer.drawLayer(
        frameCtx,
        this,
  ...
  drawEdgesColorTransparent(renderFlags, frameCtx) {
    ...
    if (this._renderers.edgesColorRenderer) {
      this._renderers.edgesColorRenderer.drawLayer(
        frameCtx,
        this,
        RENDER_PASSES.EDGES_COLOR_TRANSPARENT
      )
    }
  }
    ...
    for (let i = 0, len = indices.length; i < len; i += 3) {
      ...
      if (math.rayTriangleIntersect(rtcRayOrigin, rtcRayDir, a, b, c, closestIntersectPos)) {
        ...
        if (!gotIntersect || dist > closestDist) {
          ...
          if (worldNormal) {
            // Not that wasteful to eagerly compute - unlikely to hit >2 surfaces on most geometry
