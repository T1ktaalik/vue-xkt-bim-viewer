...
export class DTXLinesLayer {
  ...
  _createBucketGeometry(portionCfg, bucket) {
    if (bucket.indices) {
      const alignedIndicesLen =
        Math.ceil(bucket.indices.length / 2 / INDICES_EDGE_INDICES_ALIGNEMENT_SIZE) *
  ...
  finalize() {
    ...
    textureState.texturePerObjectColorsAndFlags =
      this._dataTextureGenerator.generateTextureForColorsAndFlags(
        gl,
    ...
    textureState.texturePerObjectInstanceMatrices =
      this._dataTextureGenerator.generateTextureForInstancingMatrices(
        gl,
    ...
    textureState.texturePerVertexIdCoordinates =
      this._dataTextureGenerator.generateTextureForPositions(
        gl,
        buffer.positionsCompressed,
        buffer.lenPositionsCompressed
      )
    textureState.texturePerLineIdPortionIds8Bits =
      this._dataTextureGenerator.generateTextureForPackedPortionIds(
        gl,
        buffer.perLineNumberPortionId8Bits
      )
    textureState.texturePerLineIdPortionIds16Bits =
      this._dataTextureGenerator.generateTextureForPackedPortionIds(
        gl,
        buffer.perLineNumberPortionId16Bits
      )
    textureState.texturePerLineIdPortionIds32Bits =
      this._dataTextureGenerator.generateTextureForPackedPortionIds(
        gl,
        buffer.perLineNumberPortionId32Bits
      )
    if (buffer.lenIndices8Bits > 0) {
      textureState.texturePerLineIdIndices8Bits =
        this._dataTextureGenerator.generateTextureFor8BitIndices(
          gl,
          buffer.indices8Bits,
    ...
    if (buffer.lenIndices16Bits > 0) {
      textureState.texturePerLineIdIndices16Bits =
    ...
    if (buffer.lenIndices32Bits > 0) {
      textureState.texturePerLineIdIndices32Bits =
        this._dataTextureGenerator.generateTextureFor32BitIndices(
          gl,
