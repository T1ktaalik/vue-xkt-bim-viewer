...
class OcclusionTester {
   ...
   */
  bindRenderBuf() {
    const shaderSourceHash = [
      this._scene.canvas.canvas.id,
      this._scene._sectionPlanesState.getHash()
    ].join(';')
        ...
        for (let sectionPlaneIndex = 0; sectionPlaneIndex < numSectionPlanes; sectionPlaneIndex++) {
          ...
          if (sectionPlaneUniforms) {
            ...
            if (active) {
              ...
              gl.uniform3fv(
                sectionPlaneUniforms.pos,
                getPlaneRTCPos(sectionPlane.dist, sectionPlane.dir, origin, tempVec3a)
              )
    ...
    if (!TEST_MODE) {
      ...
      for (let i = 0, len = this._occlusionLayersList.length; i < len; i++) {
        ...
        for (let i = 0; i < occlusionLayer.lenOcclusionTestList; i++) {
          ...
          const j = i * 2
          const color = this._readPixelBuf.read(
            Math.round(occlusionLayer.pixels[j] * resolutionScale),
            Math.round(occlusionLayer.pixels[j + 1] * resolutionScale)
          )
