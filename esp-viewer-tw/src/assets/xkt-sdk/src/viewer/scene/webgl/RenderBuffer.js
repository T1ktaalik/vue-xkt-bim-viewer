...
class RenderBuffer {
  ...
  _touch(...internalformats) {
    ...
    if (internalformats.length > 0) {
      colorTextures.push(
        ...internalformats.map((internalformat) =>
          this.createTexture(width, height, internalformat)
        )
      )
    } else {
  ...
  read(
    ...
    const x = pickX
    const y = this.buffer.height
      ? this.buffer.height - pickY - 1
      : this.gl.drawingBufferHeight - pickY
    ...
    if (format !== 'jpeg' && format !== 'png' && format !== 'bmp') {
      console.error(
        "Unsupported image format: '" +
          format +
          "' - supported types are 'jpeg', 'bmp' and 'png' - defaulting to 'png'"
      )
      format = 'png'
    }
