...
const Renderer = function (scene, options) {
  ...
  this.clear = function (params) {
    ...
    if (canvasTransparent) {
      ...
      const backgroundColor = scene.canvas.backgroundColorFromAmbientLight
        ? this.lights.getAmbientColorAndIntensity()
  ...
  function updateDrawlist() {
    // Prepares state-sorted array of drawables from maps of inserted drawables
    ...
    if (imageDirty) {
      // Image is usually dirty because the camera moved
      cullDrawableList()
    }
  }
    ...
    for (let type in drawableTypeInfo) {
      if (drawableTypeInfo.hasOwnProperty(type)) {
        ...
        for (let i = 0, len = drawableList.length; i < len; i++) {
          ...
          if (drawable.renderFlags.colorOpaque) {
            // Transparent objects don't cast shadows (yet)
  ...
  function drawColor(params) {
    ...
    if (saoEnabled && saoPossible) {
      const occlusionRenderBuffer1 = renderBufferManager.getRenderBuffer('saoOcclusion')
      frameCtx.occlusionTexture = occlusionRenderBuffer1
        ? occlusionRenderBuffer1.getTexture()
        : null
    } else {
    ...
    if (
      highlightedFillTransparentBinLen > 0 ||
      highlightedEdgesTransparentBinLen > 0 ||
    ...
    return function (params, pickResult = _pickResult) {
      ...
      for (let type in drawableTypeInfo) {
        if (drawableTypeInfo.hasOwnProperty(type)) {
          ...
          for (let i = 0, len = drawableList.length; i < len; i++) {
            const drawable = drawableList[i]
            if (drawable.setPickMatrices) {
              // Eg. SceneModel, which needs pre-loading into texture
      ...
      const pickedEntity = pickable.delegatePickedEntity
        ? pickable.delegatePickedEntity()
        : pickable
    ...
    for (let type in drawableTypeInfo) {
      if (drawableTypeInfo.hasOwnProperty(type)) {
        ...
        for (let i = 0, len = drawableList.length; i < len; i++) {
          ...
          if (
            !drawable.drawPickMesh ||
            (params.pickInvisible !== true && drawable.visible === false) ||
            drawable.pickable === false
          ) {
            continue
          }
  ...
  this.snapPick = (function () {
    ...
    return function (
      ...
      const vertexPickBuffer = renderBufferManager.getRenderBuffer('uniquePickColors-aabs', {
        depthTexture: true,
        size: [2 * snapRadiusInPixels + 1, 2 * snapRadiusInPixels + 1]
      })
      ...
      const snapPickNormalResultArray = vertexPickBuffer.readArray(
        gl.RGBA_INTEGER,
        gl.INT,
        Int32Array,
        4,
      ...
      const snapPickIdResultArray = vertexPickBuffer.readArray(
        gl.RGBA_INTEGER,
        gl.UNSIGNED_INT,
        Uint32Array,
        4,
      ...
      if (pickResultMiddleXY[3] !== 0) {
        const pickedLayerParmasSurface =
          layerParamsSurface[Math.abs(pickResultMiddleXY[3]) % layerParamsSurface.length]
          ...
          snapPickResult.push({
            ...
            isVertex:
              snapToVertex && snapToEdge
                ? snapPickResultArray[i + 3] > layerParamsSnap.length / 2
                : snapToVertex,
            result: [
      ...
      if (snapPickResult.length > 0) {
        ...
        snappedPickable =
          pickIDs.items[
            snapPickId[0] + (snapPickId[1] << 8) + (snapPickId[2] << 16) + (snapPickId[3] << 24)
          ]
      }
      ...
      const snappedEntity =
        snappedPickable && snappedPickable.delegatePickedEntity
          ? snappedPickable.delegatePickedEntity()
          : snappedPickable
  ...
  function gpuPickWorldNormal(
    ...
    const worldNormal = [pix[0] / math.MAX_INT, pix[1] / math.MAX_INT, pix[2] / math.MAX_INT]
      ...
      for (let type in drawableTypeInfo) {
        if (drawableTypeInfo.hasOwnProperty(type)) {
          ...
          for (let i = 0, len = drawableList.length; i < len; i++) {
            const drawable = drawableList[i]
            if (
              !drawable.drawOcclusion ||
              drawable.culled === true ||
              drawable.visible === false ||
              drawable.pickable === false
            ) {
              // TODO: Option to exclude transparent?
              continue
            }
