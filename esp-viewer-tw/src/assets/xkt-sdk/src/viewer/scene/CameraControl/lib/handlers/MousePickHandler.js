  ...
  constructor(scene, controllers, configs, states, updates) {
    ...
    canvas.addEventListener(
      ...
      (this._canvasMouseUpHandler = (e) => {
        ...
        if (
          Math.abs(e.clientX - states.mouseDownClientX) > 3 ||
          Math.abs(e.clientY - states.mouseDownClientY) > 3
        ) {
          return
        }
        ...
        if (this._clicks === 1) {
          ...
          pickController.schedulePickEntity =
            configs.doublePickFlyTo || doublePickedSubs || doublePickedSurfaceSubs
          pickController.schedulePickSurface =
            pickController.schedulePickEntity && doublePickedSurfaceSubs
          pickController.update()
