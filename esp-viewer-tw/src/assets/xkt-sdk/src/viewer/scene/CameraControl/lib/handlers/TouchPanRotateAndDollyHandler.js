...
class TouchPanRotateAndDollyHandler {
  constructor(scene, controllers, configs, states, updates) {
      ...
      (this._canvasTouchStartHandler = (event) => {
        ...
        if (touches.length === 1 && changedTouches.length === 1) {
          ...
          if (configs.followPointer) {
            ...
            if (!configs.planView) {
              if (
                pickController.picked &&
                pickController.pickedSurface &&
                pickController.pickResult &&
                pickController.pickResult.worldPos
              ) {
    ...
    canvas.addEventListener(
      'touchmove',
      (this._canvasTouchMoveHandler = (event) => {
        if (!(configs.active && configs.pointerEnabled)) {
          return
        }
        ...
        if (numTouches === 1) {
          ...
          if (
            ...
            (Math.abs(xPanDelta) > configs.longTapRadius ||
              Math.abs(yPanDelta) > configs.longTapRadius)
          ) {
          ...
          if (configs.planView) {
            ...
            if (camera.projection === 'perspective') {
              ...
              const depth = Math.abs(
                touchPicked
                  ? math.lenVec3(math.subVec3(pickedWorldPos, scene.camera.eye, []))
                  : scene.camera.eyeLookDist
              )
              const targetDistance =
                depth * Math.tan(((camera.perspective.fov / 2) * Math.PI) / 180.0)

              updates.panDeltaX +=
                ((xPanDelta * targetDistance) / canvasHeight) * configs.touchPanRate
              updates.panDeltaY +=
                ((yPanDelta * targetDistance) / canvasHeight) * configs.touchPanRate
            } else {
              updates.panDeltaX +=
                0.5 * camera.ortho.scale * (xPanDelta / canvasHeight) * configs.touchPanRate
              updates.panDeltaY +=
                0.5 * camera.ortho.scale * (yPanDelta / canvasHeight) * configs.touchPanRate
            }
          } else {
          ...
          const lastMiddleTouch = math.geometricMeanVec2(
            lastCanvasTouchPosList[0],
            lastCanvasTouchPosList[1]
          )
          ...
          if (Math.abs(dollyDelta) < 1.0) {
            ...
            if (camera.projection === 'perspective') {
              const pickedWorldPos = pickController.pickResult
                ? pickController.pickResult.worldPos
                : scene.center
