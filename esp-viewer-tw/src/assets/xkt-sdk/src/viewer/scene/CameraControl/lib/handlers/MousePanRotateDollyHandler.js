...
class MousePanRotateDollyHandler {
  constructor(scene, controllers, configs, states, updates) {
    ...
    document.addEventListener(
      'mousemove',
      (this._documentMouseMoveHandler = () => {
        if (!(configs.active && configs.pointerEnabled)) {
          return
        }
        ...
        const panning =
          keyDown[scene.input.KEY_SHIFT] ||
          configs.planView ||
          (!configs.panRightClick && mouseDownMiddle) ||
          (configs.panRightClick && mouseDownRight)
          ...
          if (!configs.planView) {
            ...
            if (configs.firstPerson) {
              ...
              updates.rotateDeltaX +=
                ((y - lastY) / canvasHeight) * (configs.dragRotationRate * 1.5)
            }
          }
        }
        ...
        switch (e.which) {
          case 3: // Right button
            ...
            if (Math.abs(x - lastXDown) < 3 && Math.abs(y - lastYDown) < 3) {
              controllers.cameraControl.fire(
                'rightClick',
                {
                  // For context menus
