...
class PanController {
  ...
  dollyToCanvasPos(optionalTargetWorldPos, targetCanvasPos, dollyDelta) {
    ...
    if (camera.projection === 'perspective') {
      ...
      camera.eye = [
        camera.eye[0] - moveVec[0],
        camera.eye[1] - moveVec[1],
        camera.eye[2] - moveVec[2]
      ]
      camera.look = [
        camera.look[0] - moveVec[0],
        camera.look[1] - moveVec[1],
        camera.look[2] - moveVec[2]
      ]
      ...
      if (optionalTargetWorldPos) {
        ...
        const eyeLookVec = math.mulVec3Scalar(
          math.normalizeVec3(math.subVec3(camera.look, camera.eye, tempVec3b)),
          lenEyeTargetVec
        )
        camera.look = [
          camera.eye[0] + eyeLookVec[0],
          camera.eye[1] + eyeLookVec[1],
          camera.eye[2] + eyeLookVec[2]
        ]
      }
      ...
      const eyeLookMoveVec = math.mulVec3Scalar(
        math.normalizeVec3(math.subVec3(camera.look, camera.eye, tempVec3a)),
        -dollyDelta,
        tempVec3b
      )
