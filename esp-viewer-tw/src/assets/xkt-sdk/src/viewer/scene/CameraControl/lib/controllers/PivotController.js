...
class PivotController {
  ...
  createPivotSphere() {
    ...
    math.decomposeMat4(
      math.inverseMat4(this._scene.viewer.camera.viewMatrix, math.mat4()),
      cameraPos,
      math.vec4(),
      math.vec3()
    )
    ...
    this._pivotSphereGeometry = new VBOGeometry(this._scene, buildSphereGeometry({ radius }))
  ...
  updatePivotElement() {
    ...
    if (this._pivoting && this._cameraDirty) {
      ...
      this._pivotCanvasPos[0] = Math.floor(
        ((1 + this._pivotProjPos[0] / this._pivotProjPos[3]) * canvasWidth) / 2
      )
      this._pivotCanvasPos[1] = Math.floor(
        ((1 - this._pivotProjPos[1] / this._pivotProjPos[3]) * canvasHeight) / 2
      )
      ...
      if (this._pivotElement) {
        this._pivotElement.style.left =
          Math.floor(canvasBoundingRect.left + this._pivotCanvasPos[0]) -
          this._pivotElement.clientWidth / 2 +
          window.scrollX +
          'px'
        this._pivotElement.style.top =
          Math.floor(canvasBoundingRect.top + this._pivotCanvasPos[1]) -
          this._pivotElement.clientHeight / 2 +
          window.scrollY +
          'px'
      }
  ...
  _cameraLookingDownwards() {
    // Returns true if angle between camera viewing direction and World-space "up" axis is too small
    const camera = this._scene.camera
  ...
  setCanvasPivotPos(canvasPos) {
    ...
    const posOnSphere = math.addVec3(
      camera.eye,
      math.mulVec3Scalar(eyeWorldPosVec, pivotShereRadius, tempVec3b),
      tempVec3c
    )
  ...
  continuePivot(yawInc, pitchInc) {
    ...
    this._azimuth += -dx * 0.01
    this._polar += dy * 0.01
    this._polar = math.clamp(this._polar, 0.001, Math.PI - 0.001)
    const pos = [
