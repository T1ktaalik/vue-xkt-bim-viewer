...
const math = {
  ...
  unglobalizeObjectId(modelId, globalId) {
    ...
    return idx === modelId.length && globalId.startsWith(modelId)
      ? globalId.substring(idx + 1)
      : globalId
  },
  ...
  fmod(a, b) {
    if (a < b) {
      console.error(
        'math.fmod : Attempting to find modulus within negative range - would be infinite loop - ignoring'
      )
      return a
    }
    while (b <= a) {
  ...
  unprojectVec3: (() => {
    ...
    return function (p, viewMat, projMat, q) {
      return this.transformVec3(
        this.mulMat4(this.inverseMat4(viewMat, mat), this.inverseMat4(projMat, mat2), mat3),
        p,
        q
      )
    }
  ...
  buildPickTriangles(positions, indices, compressGeometry) {
    ...
    const pickPositions = compressGeometry
      ? new Uint16Array(numIndices * 9)
      : new Float32Array(numIndices * 9)
  ...
  buildKDTree: (() => {
    ...
    function buildNode(triangles, indices, positions, depth) {
      ...
      for (t = 0, len = triangles.length; t < len; ++t) {
        ...
        if (
          positions[pi0 + dim] <= mid ||
          positions[pi1 + dim] <= mid ||
          positions[pi2 + dim] <= mid
        ) {
