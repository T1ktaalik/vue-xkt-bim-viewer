 ...
 * @private
 */
var compressPositions = (function () {
  ...
  return function (array, min, max) {
    ...
    let i
    for (i = 0; i < array.length; i += 3) {
      quantized[i + 0] = Math.max(
        0,
        Math.min(65535, Math.floor((array[i + 0] - min[0]) * multiplier[0]))
      )
      quantized[i + 1] = Math.max(
        0,
        Math.min(65535, Math.floor((array[i + 1] - min[1]) * multiplier[1]))
      )
      quantized[i + 2] = Math.max(
        0,
        Math.min(65535, Math.floor((array[i + 2] - min[2]) * multiplier[2]))
      )
    }
    math.identityMat4(translate)
    ...
    math.identityMat4(scale)
    math.scalingMat4v(
      [(max[0] - min[0]) / 65535, (max[1] - min[1]) / 65535, (max[2] - min[2]) / 65535],
      scale
    )
...
var compressUVs = (function () {
  ...
  return function (array, min, max) {
    ...
    math.scalingMat3v([(max[0] - min[0]) / 65535, (max[1] - min[1]) / 65535], scale)
