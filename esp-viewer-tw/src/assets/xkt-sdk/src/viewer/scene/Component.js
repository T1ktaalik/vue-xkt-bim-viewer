      ...
      for (const subId in subs) {
        if (subs.hasOwnProperty(subId)) {
          ...
          if (this._eventCallDepth < 300) {
            ...
            this.error(
              "fire: potential stack overflow from recursive event '" +
                event +
                "' - dropping this event"
            )
          }
    ...
    if (oldComponent) {
      ...
      if (onDetached) {
        if (utils.isFunction(onDetached)) {
          ...
          onDetached.scope
            ? onDetached.callback.call(onDetached.scope, oldComponent)
            : onDetached.callback(oldComponent)
        }
      }
    ...
    if (component) {
      ...
      if (onAttached) {
        if (utils.isFunction(onAttached)) {
          ...
          onAttached.scope
            ? onAttached.callback.call(onAttached.scope, component)
            : onAttached.callback(component)
        }
      }
