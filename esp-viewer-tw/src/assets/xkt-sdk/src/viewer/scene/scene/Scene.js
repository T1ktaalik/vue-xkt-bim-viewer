...
class Scene extends Component {
  ...
  _addComponent(component) {
    if (component.id) {
      ...
      if (this.components[component.id]) {
        this.error(
          'Component ' +
            utils.inQuotes(component.id) +
            ' already exists in Scene - ignoring ID, will randomly-generate instead'
        )
        component.id = null
      }
    }
  ...
  set ticksPerRender(value) {
    if (value === undefined || value === null) {
      ...
      this.error(
        "Unsupported value for 'ticksPerRender': '" +
          value +
  ...
  set ticksPerOcclusionTest(value) {
    if (value === undefined || value === null) {
      ...
      this.error(
        "Unsupported value for 'ticksPerOcclusionTest': '" +
          value +
          "' - should be an integer greater than zero."
      )
      value = 20
  ...
  set passes(value) {
    if (value === undefined || value === null) {
      ...
      this.error(
        "Unsupported value for 'passes': '" + value + "' - should be an integer greater than zero."
      )
      value = 1
    }
  ...
  get geometry() {
    return (
      this.components['default.geometry'] ||
      buildBoxGeometry(ReadableGeometry, this, {
        id: 'default.geometry',
        dontClear: true
      })
    )
  ...
  get material() {
    return (
      this.components['default.material'] ||
      new PhongMaterial(this, {
        id: 'default.material',
  ...
  get xrayMaterial() {
    return (
      this.components['default.xrayMaterial'] ||
      new EmphasisMaterial(this, {
  ...
  get highlightMaterial() {
    return (
      this.components['default.highlightMaterial'] ||
      new EmphasisMaterial(this, {
  ...
  get selectedMaterial() {
    return (
      this.components['default.selectedMaterial'] ||
      new EmphasisMaterial(this, {
  ...
  get edgeMaterial() {
    return (
      this.components['default.edgeMaterial'] ||
      new EdgeMaterial(this, {
  ...
  get pointsMaterial() {
    return (
      this.components['default.pointsMaterial'] ||
      new PointsMaterial(this, {
  ...
  get linesMaterial() {
    return (
      this.components['default.linesMaterial'] ||
      new LinesMaterial(this, {
    ...
    for (const id in this.components) {
      if (this.components.hasOwnProperty(id)) {
        ...
        if (!component._dontClear) {
          // Don't destroy components like Camera, Input, Viewport etc.
          component.destroy()
        }
      }
    }
  ...
  getAABB(ids) {
    ...
    if (utils.isString(ids)) {
      ...
      if (entity && entity.aabb) {
        // A Component subclass with an AABB
        return entity.aabb
      }
