...
class DirLight extends Light {
  ...
  constructor(owner, cfg = {}) {
    ...
    this._state = new RenderState({
      ...
      getShadowProjMatrix: () => {
        if (this._shadowProjMatrixDirty) {
      ...
      getShadowRenderBuf: () => {
        if (!this._shadowRenderBuf) {
          this._shadowRenderBuf = new RenderBuffer(this.scene.canvas.canvas, this.scene.canvas.gl, {
            size: [1024, 1024]
          }) // Super old mobile devices have a limit of 1024x1024 textures
        }
