...
function Core() {
  ...
  this.runTasks = function (until = -1) {
...
const frame = function () {
  ...
  renderScenes()
  lastTime = time
  window.requestPostAnimationFrame !== undefined
    ? window.requestPostAnimationFrame(frame)
    : requestAnimationFrame(frame)
}

function runTasks(time) {
  // Process as many enqueued tasks as we can within the per-frame task budget
