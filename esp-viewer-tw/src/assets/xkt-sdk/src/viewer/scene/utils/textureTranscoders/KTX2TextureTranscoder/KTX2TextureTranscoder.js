...
class KTX2TextureTranscoder {
  ...
  constructor({ viewer, transcoderPath, workerLimit }) {
    this._transcoderPath =
      transcoderPath || 'https://cdn.jsdelivr.net/npm/@xeokit/xeokit-sdk/dist/basis/'
  ...
  _init() {
    if (!this._transcoderPending) {
      ...
      this._transcoderPending = Promise.all([jsContent, binaryContent]).then(
      ...
      if (activeTranscoders > 0) {
        console.warn(
          'KTX2TextureTranscoder: Multiple active KTX2TextureTranscoder may cause performance issues.' +
            ' Use a single KTX2TextureTranscoder instance, or call .dispose() on old instances.'
        )
      }
      activeTranscoders++
    }
...
KTX2TextureTranscoder.BasisWorker = function () {
  ...
  function getTranscoderFormat(basisFormat, width, height, hasAlpha) {
    ...
    console.warn(
      'KTX2TextureTranscoder: No suitable compressed texture format found. Decoding to RGBA32.'
    )
