...
class PropertiesInspector extends Controller {
  ...
  clear() {
    ...
    html.push(
      `<p class="subsubtitle no-object-selected-warning">No object inspected. Right-click or long-tab an object and select \'Inspect Properties\' to view its properties here.</p>`
    )
    html.push(`</div>`)
  ...
  _setPropertySets(metaObject, propertySets) {
    ...
    if (!metaObject) {
      ...
      if (!propertySets || propertySets.length === 0) {
        html.push(
          `<p class="subtitle xeokit-no-prop-set-warning">No properties sets found for this object.</p>`
        )
        html.push(`</div>`)
      } else {
        html.push(`</div>`)
