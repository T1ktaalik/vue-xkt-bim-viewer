...
class ModelsExplorer extends Controller {
  ...
  loadModel(modelId, done, error) {
    ...
    this.bimViewer._busyModal.show(
      `${this.viewer.localeService.translate('busyModal.loading') || 'Loading'} ${modelInfo.name}`
    )
    ...
    this.server.getGeometry(
      ...
      (arraybuffer) => {
        const objectColorSource =
          modelInfo.objectColorSource || this.bimViewer.getObjectColorSource()
        const objectDefaults =
          objectColorSource === 'model' ? ModelIFCObjectColors : ViewerIFCObjectColors
