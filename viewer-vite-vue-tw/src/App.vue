<template>
  <div class="tw-absolute tw-h-[100vh] tw-w-[100vw] tw-overflow-hidden" id="viewerApp">
    <div id="ToolsTabs" class="tw-absolute tw-flex tw-w-[100vw]  tw-justify-between   ">
      <div id="myToolbar" class="tw-z-40">
        <div class="xeokit-toolbar">
          <div class="xeokit-btn-group tw-border-solid tw-border  tw-flex tw-flex-col" role="group">
            <!-- tw-bg-blue-200/30 tw-text-gray-900 tw-border-blue-200  -->
            <button>
              <span class="tw-icon-[mdi--home] xeokit-reset" style="font-size: 2rem"></span>
            </button>
            <button>
              <span class="tw-icon-[mdi--arrow-expand-all] xeokit-fit " style="font-size: 2rem"></span>
            </button>
            <button>
              <span class="tw-icon-[mdi--video-2d] xeokit-threeD " style="font-size: 2rem"> <!-- 'mdi-video-3d' --><!-- <q-btn flat @click="is2dActive = !is2dActive" :icon="is2dActive ? 'mdi-video-2d' : 'mdi-video-3d'" size="md"
                padding="xs xs" class="xeokit-threeD" ></q-btn> --></span>
            </button>

            <button><span class="xeokit-firstPerson iw-icon-[mdi--human-male]" style="font-size: 2rem">aaaaa</span></button>
            ...
            <button><span class="xeokit-ortho "></span></button>
            ...
            <q-btn v-show=false flat icon="mdi-angle-acute" size="md" padding="xs xs"
              class="xeokit-measure-angle"></q-btn>
            <q-btn flat size="md" padding="xs xs" class="xeokit-section">
              <div class="tw-flex tw-flex-col">
                <div>
                  <q-icon @click="isSectionButtonActive = !isSectionButtonActive"
                    :name="isSectionButtonActive ? 'mdi-scissors-cutting' : 'mdi-content-cut'"></q-icon>
                </div>
                <div class=" xeokit-section-menu-button ">
                  <q-icon name="mdi-menu-down" class="xeokit-section-menu-button-arrow"></q-icon>
                </div>
      ...
      <div class="tw-z-40">
        ...
        <div id="explorer">
          <div v-show="isTabPanelVisible.models"
            ...
            <div class="tw-justify-end tw-flex tw-flex-row"><q-btn @click="isTabPanelVisible.models = false" padding="xs"
                icon="close" flat size="md"></q-btn></div>
          ...
          <div v-show="isTabPanelVisible.objects"
            ...
            <div class="tw-justify-end tw-flex tw-flex-row"><q-btn @click="isTabPanelVisible.objects = false" padding="xs"
                icon="close" flat size="md"></q-btn></div>
          ...
          <div v-show="isTabPanelVisible.classes"
            ...
            <div class="tw-justify-end tw-flex tw-flex-row"><q-btn @click="isTabPanelVisible.classes = false" padding="xs"
                icon="close" flat size="md"></q-btn></div>
          ...
          <div v-show="isTabPanelVisible.storeys"
            ...
            <div class="tw-justify-end tw-flex tw-flex-row"><q-btn @click="isTabPanelVisible.storeys = false" padding="xs"
                icon="close" flat size="md"></q-btn></div>
            <div class="xeokit-i18n xeokit-tab-btn tw-text-center disabled tw-uppercase "
    ...
    <div class="tw-absolute tw-bottom-0 tw-left-1/2 tw-z-50 tw-h-30 ">
      {{ viewPoint }}
    </div>
    <div class="tw-absolute tw-bottom-0 tw-left-0 tw-z-60 ">
      <button class="tw-m-3 tw-text-red-600" id="savingBCF" ref="saveBCFButton">SaveBCF</button>
      <button class="tw-m-3" id="showingBCF" ref="showBCFButton">ShowBCF</button>
      <div>
        <div v-if="isSupported">
          <button @click="copy(JSON.stringify(viewPoint))" class="tw-text-red-800">Копируй BCF!
...
let viewPoint
const { text, copy, copied, isSupported } = useClipboard({ viewPoint })
const saveBCFButton = ref(null)
const showBCFButton = ref(null)
...
function launchViewer() {
  ...
  const savingBCF = document.getElementById("savingBCF")
  savingBCF.addEventListener('click', function () {
  ...
  const showingBCF = document.getElementById("showingBCF")
  showingBCF.addEventListener('click', function () {
    bimViewer.loadBCFViewpoint({
    "perspective_camera": {
        "camera_view_point": {
