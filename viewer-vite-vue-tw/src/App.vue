...
<label id="inspector_toggle_label" for="inspector_toggle" class="xeokit-i18n inspector_toggle_label xeokit-btn fas fa-info-circle fa-2x" data-xeokit-i18ntip="toolbar.toggleProperties" data-tippy-content="Toggle properties"></label> -->
    <button class="tw-left-5 tw- top-5 tw-absolute tw-z-30" v-on:click="isExplorerVisible = !isExplorerVisible">
      Toggle explorer
    </button>
    <button class="tw-right-5 tw- top-5 tw-absolute tw-z-30">
      Toggle inspector
    </button>
    <div
    v-show="isExplorerVisible"
      id="myExplorer"
      class="tw-absolute tw-h-screen md:tw-w-64 lg:tw-w-96 tw-w-48 tw-bg-red-200 tw-z-10 tw-left-0"
    ></div>
    <div id="myToolbar" class="tw-z-30"></div>
    <div
      id="myInspector"
      class="tw-absolute tw-h-screen md:tw-w-64 lg:tw-w-108 tw-w-48 tw-bg-orange-200 tw-z-20 tw-right-0"
    ></div>
    <div id="myViewer" class="tw-absolute tw-w-screen tw-h-screen">
      <canvas id="myCanvas" class="tw-w-full tw-h-full"></canvas>
      <canvas id="myNavCubeCanvas" class="tw-right-0 tw-bottom-0 tw-absolute"></canvas>
    </div>
  </div>
</template>
<script setup>
