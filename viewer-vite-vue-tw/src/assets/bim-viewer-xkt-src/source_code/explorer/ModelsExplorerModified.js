...
class ModelsExplorer extends Controller {
    ...
    _buildModelsMenu(projectInfo) {
        ...
        for (let i = 0, len = modelsInfo.length; i < len; i++) {
            ...
            html += "<div class='xeokit-form-check tw-p-2'>";
            html += "<input id='" + modelInfo.id  + "' type='checkbox'  value=''><span id='span-" + modelInfo.id + "' class='disabled tw-pl-2'>" + modelInfo.name + "</span>";
            html += "</div>";
        }
